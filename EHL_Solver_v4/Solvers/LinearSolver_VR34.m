function [u, v, DuDx, dudy, DvDx, dvdy, T_xx, T_xy, T_yy, p_LIN, pD_LIN] = LinearSolver_VR34(coeffs)
...
dydx = -y_MAT ./ h.^2 .* dhdx;          %Coordiante transform y' = y/h where the rest of the code uses y' as y.
dydx = -y_MAT ./ h .* dhdx;             %Modified by multiplying "h". where it come from [d(.)/dy' = h*d(.)/dy]
...
C = -(E + W);
F1 = d2hdx2 .* (-1 / 3 + 1.5 * hm ./ h - 1.5 * (hm ./ h).^2);
F2 = 1 ./ h .* dhdx.^2 .* (-1.5 * hm ./ h + 3 * (hm ./ h).^2);
F = F1 + F2;
CHECK = 0.5 * dhdx;

pD_LIN = zeros(1,N);
pD_LIN(2: end-1) = TDMA(W(2:end-1),C(2:end-1),E(2:end-1),F(2:end-1));

p_LIN = zeros(1,N);
p_LIN(2: end-1) = TDMA(W(2:end-1),C(2:end-1),E(2:end-1),CHECK(2:end-1));
...
ValueCell = {p, pD, T_xx, T_yy, T_xy,LD,LN};

% save(path + "LIN_variables" + "_" + string(N) + "x" + string(N), "ValueCell")
