function nondimensionalload(solution_matrix, savepath)
...
for j = 1:l
    ...
    for i = 1:m
        if i ~= 2
        EHLCompareL_NoNDimensional(solution_matrix(:,j,i), savepath_2in)
        end
    end
   ...
   figure(3);
   title("\bf{U = }" + string(solution_matrix(1,1,j).velocity * solution_matrix(1,1,j).domain.mu / solution_matrix(1,1,j).domain.Rx / solution_matrix(1,1,j).domain.E),"Interpreter","latex")
   ...
   saveas(f_coeff_plot, savepath_in + "friction\" +  "f_coeff_beta" + string(solution_matrix(1,j,1).viscocity_ratio) +  "U" + string(solution_matrix(1,j,1).velocity) + ".svg")
   saveas(f_coeff_plot, savepath_in + "friction\" +  "f_coeff_beta" + string(solution_matrix(1,j,1).viscocity_ratio) +  "U" + string(solution_matrix(1,j,1).velocity) + ".fig")
