function dimensionalwi(solution_matrix, savepath)
...
for j = 1:l
    ...
    for i = 1:k
        if min([solution_matrix(1,1,:).wiessenberg_Number]) < 8
            EHLCompareDe_Dimensional(solution_matrix(i, j, :), savepath_2in)
        else
            EHLCompareWi_Dimensional(solution_matrix(i, j, :), savepath_2in)
        end
    end
    ...
    for p = 1:length(solution_matrix(:,1,1))
        legend_list = [legend_list, "\bf{L = " + string(solution_matrix(p,1,1).applied_load) + "}"];
    end
   figure(3);
   title("\bf{U = }" + string(solution_matrix(1,j,1).velocity),"Interpreter","latex")
