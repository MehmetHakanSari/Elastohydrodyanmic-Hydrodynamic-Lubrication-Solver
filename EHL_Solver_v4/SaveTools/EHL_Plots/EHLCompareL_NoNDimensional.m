function EHLCompareL_NoNDimensional(solution, savepath)
...
for i = 1:length(solution(:,1))
    ...
    plot([solution(i,1).domain.x] ./ [solution(i,1).domain.bH], [solution(i,1).pressure ./ max(real(solution(i,1).hertzian_pressure))], "LineWidth",2)
    figure(2); hold on; set(gca, 'ColorOrder', renkler);
    plot([solution(i,1).domain.x] ./ [solution(i,1).domain.bH], [solution(i,1).h ./ solution(i,1).domain.href], "LineWidth",2)
    
    h = solution(i,1).h;
    hmin_list(i) = min(h);
   
    legend_list = [legend_list, "\bf{L} " + string(applied_load_list(i) ./ solution(i,1).domain.Rx ./ solution(i,1).domain.E ./ solution(i,1).domain.L)];
    applied_load_list(i) = applied_load_list(i) ./ solution(i,1).domain.Rx ./ solution(i,1).domain.E ./ solution(i,1).domain.L;
    
    f_t = [f_t solution(i,1).friction{1} * solution(i,1).domain.href / (solution(i,1).velocity * solution(i,1).domain.mu * solution(i,1).domain.L)];
    f_b = [f_b solution(i,1).friction{2} * solution(i,1).domain.href / (solution(i,1).velocity * solution(i,1).domain.mu * solution(i,1).domain.L)];
    f_v = [f_v solution(i,1).friction{3} * solution(i,1).domain.href / (solution(i,1).velocity * solution(i,1).domain.mu * solution(i,1).domain.L)];
    f_coeff = [f_coeff solution(i,1).friction{4}];
end

figure(3); hold on; box on;
set(gca, 'ColorOrder', renkler);
plot(applied_load_list,  hmin_list/(hmin_list(1)),".-","LineWidth",1.9,"MarkerSize", 24)
set(gca,'linewidth',1.1)

figure(4);
subplot(2,2,1); hold on; box on;
set(gca, 'ColorOrder', newcolors);
plot(applied_load_list, f_t, "^--","LineWidth",1.9,"MarkerSize", 24)
plot(applied_load_list, f_b,"v--","LineWidth",1.9,"MarkerSize", 24)
subplot(2,2,2); hold on
set(gca, 'ColorOrder', renkler);
plot(applied_load_list, f_coeff,".--","LineWidth",1.9,"MarkerSize", 24)
subplot(2,2,3); hold on
set(gca, 'ColorOrder', renkler);
plot(applied_load_list, f_t + f_b, ".--","LineWidth",1.9,"MarkerSize", 24)
subplot(2,2,4); hold on
set(gca, 'ColorOrder', renkler);
plot(applied_load_list, f_v, ".--","LineWidth",1.9,"MarkerSize", 24)

figure(5); hold on; box on;
set(gca, 'ColorOrder', renkler);
plot(applied_load_list, f_coeff,".-","LineWidth", 2, "MarkerSize", 24)
set(gca,'linewidth',1.1)
...
ylabel("\boldmath{$h_{min} / h_{min, " +  string(applied_load_list(1))  +"}$}","Interpreter","latex")
...
saveas(pressure_plot, savepath + "pressure/" + "EHLPressureNoNWi_" +  "beta" + string(solution(1,1).viscocity_ratio) + "Wi" + string(solution(1,1).wiessenberg_Number) + "U" + string(solution(1,1).velocity) + ".svg")
saveas(gap_plot, savepath + "gap/" + "EHLGapNoNWi_" +  "beta" + string(solution(1,1).viscocity_ratio) + "Wi" + string(solution(1,1).wiessenberg_Number) + "U" + string(solution(1,1).velocity) + ".svg")

saveas(pressure_plot, savepath + "pressure/" + "EHLPressureNoNWi_" +  "beta" + string(solution(1,1).viscocity_ratio) + "Wi" + string(solution(1,1).wiessenberg_Number) + "U" + string(solution(1,1).velocity) + ".fig")
saveas(gap_plot, savepath + "gap/" + "EHLGapNoNWi_" +  "beta" + string(solution(1,1).viscocity_ratio) + "Wi" + string(solution(1,1).wiessenberg_Number) + "U" + string(solution(1,1).velocity) + ".fig")
