function [T_xx, T_xy, T_yy, DTxxDx] = StressSolver_IE_EU(coeffs)
...
Stress(:,:,1) = zeros(N_y, N_x);%T_xx;
Stress(:,:,2) = zeros(N_y, N_x);%T_xy;
Stress(:,:,3) = zeros(N_y, N_x);%T_yy;

tic
while k
    counter = counter + 1;
    
    Stress_New = OlroydB_VR_Implicit_ADI(Stress, coeffs);
